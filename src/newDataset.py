...
class LegacyFashionIQ(torch.utils.data.Dataset):
    def __init__(self, path, category, transform=None, split='val-split'):
        ...
        # with open(os.path.join(self.caption_dir,'image_descriptions.json'),'r') as f:
        ...
        # 后添加，由LLM生成的新的修改文本
        with open(os.path.join(self.caption_dir, 'modified_captions_train.json'),'r') as f:
            self.train_modified_captions = json.load(f)
            
        # 后添加，由MLLM生成的修改后图片描述
        with open(os.path.join(self.caption_dir,'modified_train_2.json'),'r') as f:
            self.train_modified = json.load(f)
    ...
    def train_init_process(self):
        ...
        # 形成训练数据，每条训练数据包含target、candidate以及captions
    ...
    def __getitem__(self, idx):
        ...
        # modified_caption = self.train_modified_captions[idx]
        # 后添加，由LLM生成的新的修改文本
        # mod_str = modified_caption['modified_texts']
        mod_str = caption['captions']
        candidate = caption['candidate']
        target = caption['target']

        # 输出一个字典，包含target_img_data、target_img_path、mod（修改文本）、textual_query、visual_query（经过CLIP预处理后的源图像向量）、source_img_path
        out = {}
        ...
        final_key = candidate.split('_')[1] + "_" + target.split('_')[1]
        out['textual_query'] = mod_str
        out['original_text'] = self.train_captions[candidate.split('_')[1]] + ', but ' + mod_str
        out['visual_query_raw'], _ = self.get_raw_img(candidate, stage=0) # 0:train 1:test//
        _, out['source_img_path'] = self.get_img(candidate, stage=0)
        out['visual_query'], out['source_img_path'] = self.get_written_img(candidate, target, stage=0)
        # out['visual_query'], out['source_img_path'] = self.get_train_img(candidate, target, stage=0)
    ...
    # 获取图片(无处理)
    def get_raw_img(self, image_name, stage=0):
        # img_path = os.path.join(self.image_dir, image_name.split('_')[0], image_name.split('_')[1] + ".jpg")
        img_path = os.path.join(self.image_dir, image_name.split('_')[1] + ".jpg")
        with open(img_path, 'rb') as f:
            img = PIL.Image.open(f)
    ...
    # 获得写上关键词的图片，返回written_img以及img_path
    def get_written_img(self, candidate, target, stage=0):
        ...
        written_img = cv2.cvtColor(written_img, cv2.COLOR_BGR2RGB)
        written_img = written_img.astype(np.uint8)
        # 转为 PIL Image
        written_img = PIL.Image.fromarray(written_img)  # 关键步骤
        
        written_img = self.transform[stage](written_img)
        return written_img, img_path

    def get_train_img(self, candidate, target, stage=0):
        img_path = os.path.join(self.path + '/resized_train', candidate.split('_')[1] + '_' + target.split('_')[1] + ".jpg")
        train_img = cv2.imread(img_path)
        train_img = cv2.resize(train_img,(512,512))
        train_img = cv2.cvtColor(train_img, cv2.COLOR_BGR2RGB)
        train_img = train_img.astype(np.uint8)
        train_img = PIL.Image.fromarray(train_img)  # 关键步骤
        
        train_img = self.transform[stage](train_img)
        return train_img, img_path

    def get_val_img(self, candidate, target, stage=0):
        img_path = os.path.join(self.path + '/resized_val', candidate.split('_')[1] + '_' + target.split('_')[1] + ".jpg")
        val_img = cv2.imread(img_path)
        val_img = cv2.resize(val_img,(512,512))
        val_img = cv2.cvtColor(val_img, cv2.COLOR_BGR2RGB)
        val_img = val_img.astype(np.uint8)
        val_img = PIL.Image.fromarray(val_img)  # 关键步骤
        
        val_img = self.transform[stage](val_img)
        return val_img, img_path

    # 获取测试数据，text_queries字典每个包含visual_query、source_img_path、source_img_id、textual_query（拼接原文+修改文本）、target_img_id、target_img_data、target_img_path、mod
    def get_test_data(self):
        ...
        with open(os.path.join(self.caption_dir, "cap.{}.{}.json".format(self.category, 'val')), 'r') as f:
            ref_captions = json.load(f)
        #后添加，captions改为由LLM生成的新的modified_texts
        # with open(os.path.join(self.caption_dir, "modified_captions_val.json"), 'r') as f:
        #     ref_captions = json.load(f)
        with open(os.path.join(self.caption_dir, 'correction_dict_{}.json'.format(self.category)), 'r') as f:
            correction_dict = json.load(f)
        with open(os.path.join(self.caption_dir, 'image_descriptions.json'), 'r') as f:
        ...
        #     img_captions = json.load(f)
        # with open(os.path.join(self.caption_dir,'modified_texts_dress_val.json'),'r') as f:
        with open(os.path.join(self.caption_dir,'modified_val_2.json'),'r') as f:
            modified_texts = json.load(f)

        test_queries = []
        for idx in range(len(ref_captions)):
            caption = ref_captions[idx]
            mod_str = self.concat_text(caption['captions'], correction_dict)
            # mod_str = caption['modified_texts']
            candidate = caption['candidate']
            target = caption['target']
            out = {}
            out['visual_query'], out['source_img_path'] = self.get_written_img(self.category + '_' + candidate, self.category + '_' + target, stage=1)
            # out['visual_query'], out['source_img_path'] = self.get_val_img(self.category + '_' + candidate, self.category + '_' + target, stage=1)
            out['source_img_id'] = images.index(candidate)

            out['textual_query'] = img_captions[candidate] + ', but ' + mod_str
            
            # final_key = candidate+ "_" + target
            # out['textual_query'] = modified_texts[final_key]
